...
public class GomeBestSaleListWorker implements Runnable {
    private Logger log = Logger.getLogger(GomeBestSaleListWorker.class);
    public static String CODE_SEPERATOR = "->";
    private Element element;
    private CategoryElement ce;
    private File destFile;

    public GomeBestSaleListWorker(File destFile, CategoryElement ce, Element element) {
        super();
    ...
    private void addChildren(CategoryElement parent) {
        Element bdEle = element.nextElementSibling();
        if (!bdEle.hasClass("bd")) {
            return;
        }
        Elements ct2Els = bdEle.select("div.item");
        if (ct2Els.isEmpty()) {
        ...
        for (Element ct2Ele : ct2Els) {
            ...
            Elements dataEls = ct2Ele.select("h3.item-hd");
            ...
            Elements ct3Els = ct2Ele.select("div.in a[href]");
            if (ct3Els.isEmpty()) {
                continue;
            }
