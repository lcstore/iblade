...
public class YhdBestSaleListWorker implements Runnable {
    private Logger log = Logger.getLogger(YhdBestSaleListWorker.class);
    public static String CODE_SEPERATOR = "->";
    private Element element;
    private CategoryElement ce;
    private File destFile;

    public YhdBestSaleListWorker(File destFile, CategoryElement ce, Element element) {
        super();
    ...
    private void addChildren(CategoryElement parent) {
        ...
        Elements ct2Els = element.select("> div.mc dl.fore dt a[href]");
        ...
        for (Element ct2Ele : ct2Els) {
            ...
            ct2Element.setName(ct2Ele.ownText().trim());
            ct2Element.setUrl(ct2Ele.absUrl("href"));
            ...
            Elements ct3Els = ct2Ele.parent().parent().select("> dd em span a[href]");
            if (ct3Els.isEmpty()) {
                continue;
            }
