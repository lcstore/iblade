...
public class JdBestSaleListWorker implements Runnable {
    private Logger log = Logger.getLogger(JdBestSaleListWorker.class);
    public static String CODE_SEPERATOR = "->";
    private Element element;
    private CategoryElement ce;
    private File destFile;

    public JdBestSaleListWorker(File destFile, CategoryElement ce, Element element) {
        super();
    ...
    private void addChildren(CategoryElement parent) {
        Elements ct2Els = element.select("div.mc dl dt a[href]");
        ...
        for (Element ct2Ele : ct2Els) {
            ...
            Elements ct3Els = ct2Ele.parent().nextElementSibling().select("em a[href]");
            if (ct3Els.isEmpty()) {
                continue;
            }
