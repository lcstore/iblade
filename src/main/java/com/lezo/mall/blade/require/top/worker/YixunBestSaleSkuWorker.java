...
public class YixunBestSaleSkuWorker implements Runnable {
    private Logger log = Logger.getLogger(YixunBestSaleSkuWorker.class);
    ...
    public void run() {
        ...
        while (true) {
            ...
            Elements nextEls = dom.select("div.sort_page_num a.sort_page_arrow[href][title=下一页]");
            if (nextEls.isEmpty()) {
                break;
            } else {
    ...
    private String getPageUrl(String url, int pageNum) {
        ...
        Pattern oReg = Pattern.compile("/$|[/?]+YTAG=[0-9.]+$");
        Matcher matcher = oReg.matcher(url);
        if (matcher.find()) {
            String sGet = matcher.group();
            sGet = sGet.replace("/", "/all/----1-----------.html");
            url = matcher.replaceFirst(sGet);
            return url;
        }
        url = url.replaceFirst("[\\-]+.html", "---1-----------.html");
        return url;
    }

    private List<TopBucket> getDataList(Document dom) {
        Elements rowEls = dom.select("ul#itemList.goods_ul li.goods_li");
        if (rowEls.isEmpty()) {
        ...
        for (Element ele : rowEls) {
            ...
            Elements titleEls = ele.select("div.mod_goods_info p.mod_goods_tit a[href][title][target=_blank]");
