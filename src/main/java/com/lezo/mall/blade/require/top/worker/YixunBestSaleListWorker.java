...
public class YixunBestSaleListWorker implements Runnable {
    private Logger log = Logger.getLogger(YixunBestSaleListWorker.class);
    public static String CODE_SEPERATOR = "->";
    private Element element;
    private CategoryElement ce;
    private File destFile;

    public YixunBestSaleListWorker(File destFile, CategoryElement ce, Element element) {
        super();
    ...
    private void addChildren(CategoryElement parent) {
        ...
        for (int i = element.elementSiblingIndex(); i < elements.size(); i++) {
            Element ct2Ele = elements.get(i);
            CategoryElement ct2Element = new CategoryElement();
            Elements ct2Els = ct2Ele.select("div.hd a.unit[href]");
            ct2Element.setName(ct2Els.first().ownText().trim());
            ct2Element.setUrl(ct2Els.first().absUrl("href"));
            ...
            Elements ct3Els = ct2Ele.select("div.bd a[target][href]");
            if (ct3Els.isEmpty()) {
                continue;
            }
