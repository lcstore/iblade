...
@Log4j
public class GomeSkuMonitor {

    public static void main(String[] args) throws Exception {
        int index = 0;
        long timeout = 5000;
        String pattern = "MMdd_HH_mm_ss";
        String dataDir = "./data/" + GomeSkuMonitor.class.getSimpleName().toLowerCase();
        disableSSLCertCheck();
        String dateChars = DateFormatUtils.format(new Date(), pattern);
        int lastErrorCount = 0;
        while (true) {
            index++;
            String url = "http://item.gome.com.cn/9123392665-1112530025.html?t=" + System.currentTimeMillis();
    ...
    private static boolean doValidateResponse(Response resp) {
        ...
        containList.add("海尔(Haier) XQB70-M1269S 7公斤 波轮全自动洗衣机(月光灰) 强力去污 额外漂洗");
        containList.add("pid=\"9123392665\"");
