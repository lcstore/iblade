...
@Log4j
public class JdPriceMonitor {

    public static void main(String[] args) throws Exception {
        int index = 0;
        long timeout = 5000;
        String pattern = "MMdd_HH_mm_ss";
        String dataDir = "./data/" + JdPriceMonitor.class.getSimpleName().toLowerCase();
        disableSSLCertCheck();
        String dateChars = DateFormatUtils.format(new Date(), pattern);
        int lastErrorCount = 0;
        while (true) {
            index++;
            String url = "http://p.3.cn/prices/get?skuid=J_547774&type=1&area=1_72_2799&callback=cnp";
    ...
    private static boolean doValidateResponse(Response resp) {
        ...
        containList.add("J_547774");
        containList.add("87.99");
