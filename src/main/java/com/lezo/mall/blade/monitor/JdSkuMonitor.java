...
@Log4j
public class JdSkuMonitor {

    public static void main(String[] args) throws Exception {
        int index = 0;
        long timeout = 5000;
        String pattern = "MMdd_HH_mm_ss";
        String dataDir = "./data/" + JdSkuMonitor.class.getSimpleName().toLowerCase();
        disableSSLCertCheck();
        String dateChars = DateFormatUtils.format(new Date(), pattern);
        int lastErrorCount = 0;
        while (true) {
            index++;
            String url = "http://item.jd.com/547774.html?t=" + System.currentTimeMillis();
    ...
    private static boolean doValidateResponse(Response resp) {
        ...
        String sShopUrl = "http://mall.jd.com/index-1000003663.html";
        String sName = "清扬(CLEAR)男士洗发露 清爽控油型(去屑+控油)750ml";
        String listId = "data-id=\"547774\"";
        int index = -1;
        index = body.indexOf(listId);
        if (index < 0) {
            return false;
        }
        index = body.indexOf(sName);
        if (index < 0) {
            return false;
        }
        index = body.indexOf(sShopUrl);
        if (index < 0) {
            return false;
        }
        return true;
