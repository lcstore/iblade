...
@Log4j
public class YhdSkuMonitor {

    public static void main(String[] args) throws Exception {
        int index = 0;
        long timeout = 5000;
        String pattern = "MMdd_HH_mm_ss";
        String dataDir = "./data/" + YhdSkuMonitor.class.getSimpleName().toLowerCase();
        disableSSLCertCheck();
        String dateChars = DateFormatUtils.format(new Date(), pattern);
        int lastErrorCount = 0;
        while (true) {
            index++;
            String url = "http://item.yhd.com/item/21008697?t=" + System.currentTimeMillis();
    ...
    private static boolean doValidateResponse(Response resp) {
        ...
        containList.add("MEI Long Hu/梅龙虎 一级西湖龙井 125g/罐");
        containList.add("pmId:21008697");
