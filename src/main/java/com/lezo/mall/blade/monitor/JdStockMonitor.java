...
@Log4j
public class JdStockMonitor {

    public static void main(String[] args) throws Exception {
        int index = 0;
        long timeout = 5000;
        String pattern = "MMdd_HH_mm_ss";
        String dataDir = "./data/" + JdStockMonitor.class.getSimpleName().toLowerCase();
        disableSSLCertCheck();
        ...
        while (true) {
            index++;
            String url =
                    "http://c0.3.cn/stock?skuId=547774&venderId=0&cat=1316,1386,11922&area=2_2813_51976_0&buyNum=1&extraParam={%22originid%22:%221%22}&ch=1&callback=getStockCallback";
    ...
    private static boolean doValidateResponse(Response resp) {
        ...
        String body = "";
        try {
            body = new String(resp.bodyAsBytes(), "GBK");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            body = "";
        }
        List<String> containList = new ArrayList<String>();
        containList.add("547774");
        containList.add("StockStateName");
        containList.add("现货");
    ...
    private static Response doRequest(String url, int maxRetry) {
        ...
        for (int i = 1; i <= maxRetry; i++) {
            try {
                return Jsoup.connect(url).header("Content-Type", "application/json").cookies(cookies)
                        .method(Method.GET).userAgent(userAgent).ignoreContentType(true)
                        .execute();
            } catch (Exception e) {
                try {
                    TimeUnit.MILLISECONDS.sleep(timeout);
